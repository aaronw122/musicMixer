...
| False | True | `ab_autolvl_tune_v1` tuning: max_boost=1.5 dB, max_cut=2.5 dB, window_sec=4.0 (**Intentional change from deployed values:** changes `max_boost_db` from 1.0->1.5 and `max_cut_db` from 2.0->2.5 to allow more dynamic range correction. This is a deliberate tweak. **Rollback guidance:** If A/B results show over-correction (pumping, loss of dynamics), revert to original values: `max_boost_db=1.0`, `max_cut_db=2.0`, `window_sec=4.0`. The original values were more conservative and may be preferable for compressed source material like hip-hop.) |
...
**Step 7.75** — Broad preset EQ only (before tempo stretch, after vocal bandpass). **Ordering note: pre-filter (bandpass at step 7.7) and corrective EQ (HPF + cuts at step 7.75) MUST run before vocal compression (step 11).** This ensures the compressor does not react to sub-bass rumble or low-frequency artifacts that will be filtered out anyway, which would waste compression headroom and cause audible pumping on low-frequency content. The pipeline ordering already enforces this (7.7 -> 7.75 -> 11), but implementers should preserve this sequence. Resonance notch cuts are deferred to after stretch to avoid ringing artifacts (see eq.py note):
```python
...
# CRITICAL: detector_audio must be set to the post-ducked instrumental bus to avoid
# the volume-dip regression (the ~11s/~22s dip bug). Without this, auto_level uses
# the mixed signal for detection, which causes vocals to trigger gain reduction on
# themselves. Use the post-ducked bus (after spectral ducking at step 12.5) rather
# than the pre-ducked instrumental_bus, so the detector sees the actual instrumental
# levels that will appear in the final mix. This prevents the auto-leveler from
# boosting during ducked passages and then over-cutting when ducking releases.
auto_level_kwargs["detector_audio"] = instrumental_bus  # post-ducked; see step 12.5
...
### Flag Combinatorics Note

With 4 sound quality flags (`ab_per_stem_eq_v1`, `ab_resonance_detection_v1`, `ab_multiband_comp_v1`, `ab_static_mastering_v1`) plus `vocal_sub_separation` and `ab_autolvl_tune_v1`, there are **64 possible flag combinations** (2^6). The A/B test suite only exercises 2 configurations (all-off control and all-on enhanced). This leaves many interaction paths untested.

**Recommendation: targeted integration tests for high-risk flag combinations.** Rather than testing all 64 combinations (impractical), add integration tests for these specific high-risk interactions:

1. **Multiband + vocal sub-separation:** Compression stacking on backing vocals (2.0:1 single-band + ~2.5:1 multiband mid-band). Verify backing vocal dynamics are not over-compressed.
2. **EQ + resonance detection:** Verify resonance notch cuts don't conflict with preset EQ cuts at similar frequencies (especially 200-400Hz range where both operate).
3. **Static mastering + multiband:** The mastering chain's LUFS normalization + limiter follows multiband compression. Verify the multiband's internal gain staging (output LUFS restoration) does not fight the mastering chain's LUFS target.
4. **Auto-leveler + multiband + vocal sub-separation (3-way):** The auto-leveler was calibrated for single-vocal-stem energy. With sub-separation, the vocal bus energy profile changes. With multiband, the auto-leveler is constrained to 1.0 dB max_boost. Verify these three features interact correctly.
5. **EQ only (no multiband, no mastering):** Verify that corrective EQ alone does not push levels beyond the standard limiter chain's expectations.

These 5 targeted tests cover the highest-risk interaction paths without requiring exhaustive combinatorial coverage. Add to `test_pipeline_wiring.py` alongside the auto-leveler 4-state tests.
